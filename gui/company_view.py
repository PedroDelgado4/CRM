...
class CompanyView(ctk.CTkFrame):
    ...
    def update_header_arrows(self):
        for key, btn in self.header_buttons.items():
            # obtener el nombre de la columan
    ...
    def refresh_list(self):
        ...
        for comp in companies:
            ...
            ctk.CTkButton(actions, text="", image=self.img_delete, width=28, height=28, 
                          fg_color=self.color_brick, hover_color="#7A1F1F",
                          command=lambda c_id=comp[0]: self.remove_company(c_id)).pack(expand=True)
    ...
    def open_add_company_window(self):
        # evitar abrir multiples ventanas
